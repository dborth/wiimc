...
static int read_header(AVFormatContext *s)
{
    ...
    switch (codec) {
    case 0: codec = CODEC_ID_PCM_S8;    break;
    case 1: codec = b->little_endian ?
                    CODEC_ID_PCM_S16LE :
                    CODEC_ID_PCM_S16BE; break;
    case 2: codec = b->little_endian ?
                    CODEC_ID_ADPCM_THP_LE :
                    CODEC_ID_ADPCM_THP;        break;
    default:
	...
	//valSTM = avio_rb32(s->pb);
	//valSTM = st->codec->sample_rate;
	
    if (!bfstm)
        avio_skip(s->pb, 2); // padding

    if (loop) {
		loop_st_point = read32(s);
        if (av_dict_set_int(&s->metadata, "loop_start",
                            av_rescale(loop_st_point, AV_TIME_BASE,
                                       st->codec->sample_rate),
                            0) < 0)
            return AVERROR(ENOMEM);
		loop_st_point /= st->codec->sample_rate;
    } else {
        avio_skip(s->pb, 4);
    }
    ...
    if (codec == CODEC_ID_ADPCM_THP || codec == CODEC_ID_ADPCM_THP_LE) {
        int ch;
        ...
        if (!bfstm) {
            avio_skip(s->pb, pos + toffset - avio_tell(s->pb) - 8LL * (st->codec->channels + 1));
            ...
            qsort(b->offsets, st->codec->channels, sizeof(*b->offsets), sort_offsets);
        }
		...
		//gal_main_save->starbits = (gal_main_save->starbits>>8) | (gal_main_save->starbits<<8);
		//16 le this for citra
        for (ch = 0; ch < st->codec->channels; ch++) {
		...
		//swapping the info channel tables fixes LE adpcm playback
		if(citra == 1) {
			int i = 0;
			for (i = 0; i < (32*st->codec->channels); i+=2) {
				int bk1 = b->table[i+1];
				int bk2 = b->table[i];
				b->table[i+1] = bk2;
				b->table[i]   = bk1;
    ...
    while (!url_feof(s->pb)) {
        ...
        case MKTAG('S','E','E','K'):
        case MKTAG('A','D','P','C'):
            if (codec != CODEC_ID_ADPCM_THP &&
                codec != CODEC_ID_ADPCM_THP_LE)
                goto skip;


		//	valSTM = b->block_count * st->codec->channels * 4;
            ...
            if (b->adpc) {
                ...
                if (bfstm && codec != CODEC_ID_ADPCM_THP_LE) {
                   ...
                   /* int i;
					...
					//now I need this for the LE hack
					if(citra == 1) {
						int i;
                        for (i = 0; i < asize; i += 2) {
                            b->adpc[i+1] = avio_r8(s->pb);
					...
					//Super confused, but adding this fixes the constant pops
        ...
        case MKTAG('D','A','T','A'):
            if ((start < avio_tell(s->pb)) ||
             //   (!b->adpc && codec == CODEC_ID_ADPCM_THP)) {
                (!b->adpc && (codec == CODEC_ID_ADPCM_THP ||
                              codec == CODEC_ID_ADPCM_THP_LE))) {
                ret = AVERROR_INVALIDDATA;
                goto fail;
			...
			if (bfstm && (codec == CODEC_ID_ADPCM_THP ||
                          codec == CODEC_ID_ADPCM_THP_LE))
                avio_skip(s->pb, 24);

			//codec = CODEC_ID_ADPCM_THP;
			//valSTM = avio_rb32(s->pb);
			//something that I know for certain, both cstm and fstm end up in the same pos
			//when it's the same data order, so, something else is the problem.
...
static int read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (codec->codec_id == CODEC_ID_ADPCM_THP ||
        codec->codec_id == CODEC_ID_ADPCM_THP_LE) {
        uint8_t *dst;
      ...
      //  bytestream_put_be32(&dst, samples);
        if (codec->codec_id == CODEC_ID_ADPCM_THP_LE) {
