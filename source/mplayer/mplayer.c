...
bool IsLoopAvi(char *_file)
{
	...
	if(!strcmp("loop.avi",&_file[i])) return true;
...
void update_osd_msg(void) {
    ...
    if((msg = get_osd_msg())) {
        if(strcmp(osd_text,msg->msg)) {
            ...
            if(mpctx->sh_video)vo_osd_changed(OSDTYPE_OSD);
            else if(term_osd) mp_msg(MSGT_CPLAYER,MSGL_STATUS,"%s%s\n",term_osd_esc,msg->msg);
        }
        return;
    }
    if(mpctx->sh_video) {
...
{		
	if(error_playing==0 && check_play_next_file(fileplaying,next_filename))
	{
		update_osd_msg();
		clear_osd_msgs();
		vo_osd_changed(OSDTYPE_SUBTITLE);
