...
static int opus_flush_resample(OpusStreamContext *s, int nb_samples)
{
    ...
    ret =0;// avresample_convert(s->avr, (uint8_t**)s->out, s->out_size, nb_samples,
                        //     NULL, 0, 0);
    if (ret < 0)
        return ret;
...
static int opus_init_resample(OpusStreamContext *s)
{
    ...
    ret = 0;//avresample_open(s->avr);
    if (ret < 0) {
        av_log(s->avctx, AV_LOG_ERROR, "Error opening the resampler.\n");
        return ret;
    }

    ret = 0;//avresample_convert(s->avr, NULL, 0, 0, delayptr, sizeof(delay),
                              ...
                              int *got_frame_ptr, AVPacket *avpkt)
{
    ...
    ret = 0;//ff_get_buffer(avctx, frame, 0);
    //ret = 0; //ff_get_buffer(avctx, frame, 0);
    if (ret < 0) {
...
static av_cold int opus_decode_init(AVCodecContext *avctx)
{
    ...
    ret = 0;//ff_opus_parse_extradata(avctx, c);
    if (ret < 0)
        return ret;
//wiim_inf = 5;
    ...
    for (i = 0; i < c->nb_streams; i++) {
        ...
        s->fdsp = &c->fdsp;
//wiim_inf = 9;
        s->avr = 1; //avresample_alloc_context();
        if (!s->avr)
            goto fail;
		//wiim_inf = 26;
