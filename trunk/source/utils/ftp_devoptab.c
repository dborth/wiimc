...
static int ftp_get_response(ftp_env* env)
{
	...
	if(i < 0)
	{
		NET_PRINTF("FTP RESPONCE: NULL", 0 );
		return i;
	}

	NET_PRINTF("FTP RESPONCE: %s\n", buf );
	...
	if(buf[3] == '-')
	{
		...
		if(b != buf + 3)
		{
			NET_PRINTF("FTP RESPONCE: bad responce\n", 0 );
			return -1;
		}
	}
...
bool net_printf( const char *fmt, ... )
{
	...
	if (debug_sock == INVALID_SOCKET)
	{
		...
		debug_sock = net_socket(AF_INET, SOCK_STREAM, IPPROTO_IP);
		if(debug_sock==INVALID_SOCKET) return false;

		set_blocking( debug_sock, true );

		t1=ticks_to_millisecs(gettime());
		do
		{
			ret = net_connect(debug_sock,(struct sockaddr*)&server_addr,sizeof(server_addr));
			t2=ticks_to_millisecs(gettime());
			usleep(1000);
			if(t2-t1 > NET_TIMEOUT) break; // 10 secs to try to connect to handle->server_addr
		} while(ret!=-EISCONN);
	...
	while(len>0)
	{
		ret=net_send(debug_sock,data,len,0);
		if(ret==-EAGAIN)
		{
			t2=ticks_to_millisecs(gettime());
			if( (t2 - t1) > NET_TIMEOUT)
